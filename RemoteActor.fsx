    ...
    let rec loop() = actor {
        ...
        | "Tweet" ->
            ...
            if userId < numNodes then
                ...
                tweetsByUser <- Array.concat [| tweetsByUser ; [|tweetString|] |]
                ...
                for hashtags in userHashtags do
                    if tweetsHastags.ContainsKey(hashtags) then
                        ...
                        thistweetsHastags <- Array.concat [| thistweetsHastags ; [|tweetString|] |]
                ...
                for mentioned in userMentions do
                    ...
                    if mentionedId < numNodes then
                        ...
                        allfollowing <- Array.concat [| allfollowing ; [|mentionedId|] |]
        ...
        | "Subscribe" ->
            ...
            if userId < numNodes then
                ...
                if userfollowing.Length < numNodes - 2 then
                    ...
                    userfollowing <- Array.concat [| userfollowing ; [|allUsers.[randomNewSub]|] |] 
        ...
        | "GoOffline" ->
            ...
            loggedOutUsers <- Array.concat [| loggedOutUsers ; [|userId|] |] 
            let destinationRef = select ("akka.tcp://Twitter@127.0.0.1:9001/user/User"+ (userId |> string)) system
